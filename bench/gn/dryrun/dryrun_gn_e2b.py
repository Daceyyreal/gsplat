"""CPU dry run of kaggle/gn_e2b_scene.py (E2b, PREREG Amendment 9 b) and of the E2b notebook's cells.
...
(3) E2b on the restored layout: 14 rows per scene in order, the CV rows scored on the odd train views
    and the full-M rows evaluated like E2's, the warm-start and M sources (E2's caches; on flowers the
    fallbacks: the run-5 cache, a reclustering and a recomputed M), the lifted checks per metric, the
    floored and unfloored objectives;
(4) resume runs nothing again;
(5) rho = 0 is E2's GN-VQ: the same warm start and M through ``floored_metric(M, 0)`` reproduce E2's
    toy report exactly, and ``report_metrics`` changes no result;
...
(7) the notebook's E2b cell (``e2b.judge_e2b`` against the toy E2 rows, rho_cv checked independently)
    and its bundle cell.
...
KDEF = fe.KDEF  # 64
VQ_ITERS = 5
DATASET = {"treehill": "mipnerf360", "flowers": "mipnerf360", "train": "tandt", "garden": "mipnerf360"}
...
assert set(ns0["SCENES"]) == set(e2b.SCENES) and ns0["SCENES"][0] == "train"
...
print("(0) E2b notebook: cells compile in order; 4 scenes, train first, pinned to Amendment 7 / run 5; "
...
for scene in e2b.SCENES:
    ...
    a = ["--scene", scene, "--dataset", DATASET[scene], "--benchmark_sh", bench_sh(scene),
         ...
         # train has no run-4 cache in E2 either: its K = 65,536 codebook lives in E2's work dir
         "--run3_kmeans_dir", "" if scene == "train" else env["km_dir"],
...
for s, run in (("treehill", "run4"), ("flowers", "run4"), ("garden", "run3")):
...
assert set(found["run5_kmeans"]) == {"treehill", "garden", "flowers"}
assert set(found["e2_work"]["train"]) == {16, 64} and set(found["e2_work"]["treehill"]) == {16}
...
assert sorted(os.listdir(os.path.join(rns["WARM_DIR"]["train"]))) == ["e2_work"]
...
WARM_EXPECTED = {"treehill": ("e2_work_cache", "e2_kmeans_cache"), "garden": ("e2_work_cache", "e2_kmeans_cache"),
                 "train": ("e2_work_cache", "e2_work_cache"), "flowers": ("recomputed", "run5_kmeans_cache")}
...
for scene in e2b.SCENES:
    ...
    assert all(v["pass"] for v in meta["lifted_checks"].values()) and len(meta["lifted_checks"]) == 7
    ...
    for r in rows:
        ...
        if cfg == e2b.CV:
            ...
            assert rho > 0 and r["metric_views"] == "all" and r["measured_odd_clamped"] == ""
            for col in ("PSNR", "train_PSNR", "measured_train_clamped", "measured_test_clamped", "shn_only_PSNR"):
                assert r[col] not in ("", "nan"), (scene, cfg, col)
print("(3) E2b: 14 rows per scene (per K the 4 CV rows, then the 3 full-M rows), CV scored on the 2 odd "
      "views with M from the 3 even ones, full-M rows evaluated like E2's; warm starts and M from E2's "
      "caches, and on flowers from the fallbacks (run-5 cache, reclustering, recomputed M); 7 lifted "
      "checks per scene; floored objective above the unfloored one for rho > 0, equal at rho = 0: ok")
...
print("(5) rho = 0 reproduces E2's toy GN-VQ report exactly (every objective in its history), and "
      "report_metrics changes neither the codebook nor the labels: ok")
...
assert res["verdict"] in ("works", "does not work") and res["criterion"]["missing"] == [], res["criterion"]
for s in e2b.SCENES:
    ...
    for k in KS:
        ...
        assert v["missing"] == [] and v["full"]["0"]["source"] == "e2_gn_vq_row"
        e2gv = next(r for r in e2_rows if r["scene"] == s and r["config"] == g2.GNVQ and int(r["n_clusters"]) == k)
        assert v["full"]["0"]["PSNR"] == float(e2gv["PSNR"])
...
for f in ["gn2b/gn2b_e2b.json", "gn2b/gn2b_rho.png", "gn2b/gn_e2b_train_log_tail.json"] + \
...
assert len([n for n in names if n.startswith("gn2b/gn2b_gn_vq_floor")]) == 14 * 4
rns["JOB_FAILED"] = ["gn_e2b_train"]
try:
    exec(bundle_cell, rns)
    raise AssertionError("the bundle cell did not raise for a failed job")
except RuntimeError as e:
    assert "gn_e2b_train" in str(e)
print(f"(7) E2b cell -> {res['verdict']!r} on the toy (complete; rho_cv the odd-view argmin everywhere, rho = 0 "
      f"read from E2's gn_vq rows), plot; bundle {len(names)} files, the stray E2 file skipped, raises last on a "
      "failed job: ok")
